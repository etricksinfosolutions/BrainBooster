...
export function dirFromDelta(dx, dy, min = 12) {
...
export function collapse(cells, matched, n, kindCount) {
    ...
    for (let c = 0; c < n; c++) {
        ...
        for (let r = n - 1; r >= 0; r--) {
            const i = r * n + c;
            if (!matched.has(i))
                survivors.push(cells[i]);
        }
        for (let r = n - 1, k = 0; r >= 0; r--)
            next[r * n + c] = k < survivors.length ? survivors[k++] : rand(kindCount);
    }
    return next;
}
/** True if ANY single adjacent swap would create a match. */
export function hasLegalMove(cells, n) {
    for (let r = 0; r < n; r++)
        for (let c = 0; c < n; c++) {
            const i = r * n + c;
            if (c < n - 1) {
...
export function freshBoard(n, kindCount) {
    let cells = [];
    let guard = 0;
    do {
        ...
        while (m.size && g2++ < 80) {
            cells = collapse(cells, m, n, kindCount);
            m = findMatches(cells, n);
        }
 ...
 */
export function resolveSwap(cells, n, i, j, kindCount) {
    const swapped = cells.slice();
    [swapped[i], swapped[j]] = [swapped[j], swapped[i]];
    if (!findMatches(swapped, n).size)
        return null;
    let cur = swapped, gained = 0, guard = 0;
    let m = findMatches(cur, n);
    while (m.size && guard++ < 80) {
        gained += m.size;
        cur = collapse(cur, m, n, kindCount);
        m = findMatches(cur, n);
    }
    if (!hasLegalMove(cur, n))
        cur = freshBoard(n, kindCount); // never hand back a dead board
