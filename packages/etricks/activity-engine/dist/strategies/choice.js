...
function layoutChoices(choices, correctIndex, config, rng) {
    if (!config.shuffle)
        return { choices: choices.slice(), correctIndex };
    const order = shuffle(choices.map((_, i) => i), rng);
    return {
        choices: order.map((i) => choices[i]),
        correctIndex: order.indexOf(correctIndex),
    };
}
function gradeChoice(prepared, response) {
...
export const multipleChoiceStrategy = {
...
export const imageQuizStrategy = {
    type: "image-quiz",
...
export const audioQuizStrategy = {
    type: "audio-quiz",
