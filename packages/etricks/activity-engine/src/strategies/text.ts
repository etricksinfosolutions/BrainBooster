...
export const fillBlankStrategy: ActivityStrategy<
  FillBlankContent,
  { segments: string[]; blanks: z.infer<typeof Blank>[] },
  FillBlankResponse
> = {
  type: "fill-blank",
  contentSchema: FillBlankContent.refine(
    (c) => (c.template.match(BLANK_MARKER)?.length ?? 0) === c.blanks.length,
    { message: "template must contain exactly one {{}} per blank", path: ["template"] },
  ),
  prepare(content) {
  ...
  grade(prepared, response) {
    ...
    let correctUnits = 0;
    const perBlank: boolean[] = prepared.blanks.map((blank, i) => {
      const given = values[i];
      if (given === undefined) return false;
      const ok = blank.answers.some(
        (a) => normalise(a, blank.caseSensitive) === normalise(given, blank.caseSensitive),
      );
      if (ok) correctUnits++;
      return ok;
    });
...
export const typingChallengeStrategy: ActivityStrategy<
