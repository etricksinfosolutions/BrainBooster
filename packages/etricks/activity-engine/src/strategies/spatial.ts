...
function layoutWordSearch(words: string[], size: number, rng: Rng): PreparedWordSearch {
  ...
  const fits = (word: string, row: number, col: number, dir: "across" | "down"): boolean => {
    for (let i = 0; i < word.length; i++) {
      ...
      const cell = grid[r]![c]!;
      if (cell !== null && cell !== word[i]) return false; // allow overlap on shared letters
    }
    return true;
  };
  ...
  for (const word of upper) {
    ...
    for (let attempt = 0; attempt < 60 && !placed; attempt++) {
      const dir: "across" | "down" = rng() < 0.5 ? "across" : "down";
    ...
    if (!placed) {
      outer: for (const dir of ["across", "down"] as const) {
        const maxStart = size - word.length;
    ...
    if (!placed) {
      throw new Error(
        `word-search: cannot place "${word}" in a ${size}×${size} grid; increase size`,
      );
    }
  }

  const filled: string[][] = grid.map((rowArr) =>
    rowArr.map((cell) => cell ?? ALPHABET[Math.floor(rng() * ALPHABET.length)]!),
  );
  return { size, grid: filled, placements, words: upper };
}

export const wordSearchStrategy: ActivityStrategy<
  WordSearchContent,
  PreparedWordSearch,
  WordSearchResponse
> = {
  ...
  grade(prepared, response): StrategyGrade {
    ...
    const found = new Set(
      (response?.found ?? []).map((w) => w.toUpperCase()).filter((w) => target.has(w)),
    );
...
export const hotspotStrategy: ActivityStrategy<
  HotspotContent,
  { image: HotspotContent["image"]; prompt: string; targets: z.infer<typeof Region>[] },
  HotspotResponse
> = {
  type: "hotspot",
  ...
  grade(prepared, response) {
    ...
    const hit = (t: z.infer<typeof Region>) =>
      points.some((p) => p.x >= t.x && p.x <= t.x + t.w && p.y >= t.y && p.y <= t.y + t.h);
...
export const puzzleGridStrategy: ActivityStrategy<
  PuzzleGridContent,
  PreparedPuzzleGrid,
  PuzzleGridResponse
> = {
