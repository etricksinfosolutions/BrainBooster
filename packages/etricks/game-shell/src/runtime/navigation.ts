 ...
 */
export const LEGAL_TRANSITIONS: Readonly<Record<Screen, readonly Screen[]>> = {
  splash: ["loading"],
...
export const INITIAL_SCREEN: Screen = "splash";
...
export function routeAfterBoot(opts: {
  ...
  if (opts.hasSession) return "home";
  if (opts.onboardingEnabled && !opts.onboardingSeen) return "onboarding";
  if (opts.loginEnabled) return "login";
  return "home";
}
