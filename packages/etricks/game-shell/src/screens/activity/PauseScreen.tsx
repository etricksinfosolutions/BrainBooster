...
export function PauseScreen(): ReactNode {
  ...
  const button = (variant: "primary" | "ghost" | "danger"): CSSProperties => ({
    width: "100%",
    ...
    color:
      variant === "primary" ? "var(--accent-ink)" : variant === "danger" ? "var(--bad)" : "var(--ink)",
  });
  ...
  return (
    ...
    >
      ...
      >
        <h1 style={{ margin: "0 0 4px", fontSize: "1.5rem", fontWeight: 800 }}>Paused</h1>
        <p style={{ margin: "0 0 8px", color: "var(--dim)", fontSize: "0.95rem" }}>
          Take a breath — {branding.displayName} will be right here.
        </p>

        <button type="button" style={button("primary")} onClick={() => back()} aria-label="Resume">
          Resume
        </button>
        <button
          type="button"
