...
function Confetti({ enabled }: { enabled: boolean }): ReactNode {
  ...
  return (
    ...
    >
      <style>
        {"@keyframes gs-confetti-fall{0%{transform:translateY(-12%) rotate(0);opacity:1}" +
          "100%{transform:translateY(115vh) rotate(720deg);opacity:.85}}"}
      </style>
      {pieces.map((p, i) => (
        <span
          key={i}
...
export function VictoryScreen(): ReactNode {
  ...
  ].filter((r): r is { icon: string; label: string; amount: number } => r.amount != null && r.amount > 0);
  ...
  return (
    ...
    >
      <Confetti enabled={!a11y.reducedMotion} />
      ...
      <h1 style={{ margin: 0, fontSize: "2rem", fontWeight: 800, position: "relative" }}>
        Level complete!
      </h1>
      {branding.mascot ? (
        <p style={{ margin: 0, color: "var(--dim)" }}>{branding.mascot.name} is so proud of you!</p>
      ) : null}

      <div role="img" aria-label={`${stars} of 3 stars`} style={{ display: "flex", gap: 12 }}>
        {[1, 2, 3].map((i) => (
          <span
            key={i}
            aria-hidden="true"
      ...
      {earned.length > 0 ? (
        ...
        >
          {earned.map((r) => (
            <span
              key={r.label}
              aria-label={`Earned ${r.amount} ${r.label}`}
              style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 6,
            ...
            >
              <span aria-hidden="true">{r.icon}</span> +{r.amount}
            </span>
          ))}
        </div>
      ) : null}
      ...
      >
        <button type="button" style={button("primary")} onClick={() => navigate("reward")}>
          Continue
        </button>
        <button type="button" style={button("ghost")} onClick={() => go("world-select", "home")}>
          Back to map
        </button>
      </div>
    </div>
