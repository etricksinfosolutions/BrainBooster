    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {subtitle ? (
            <p style={{ margin: "2px 0 0", color: "var(--dim)", fontSize: ".9rem" }}>{subtitle}</p>
          ) : null}
        </div>
        {hud ? <WalletHud compact /> : null}
      </header>
      <main
        style={{
          flex: 1,
 ...
 */
export function WalletHud({ compact = false }: { compact?: boolean }): ReactNode {
  ...
  return (
    ...
    >
      <Chip icon={skin.soft.icon} label={skin.soft.label} value={wallet.coins} />
      <Chip icon={skin.hard.icon} label={skin.hard.label} value={wallet.diamonds} />
      {!compact && <Chip icon={skin.xp.icon} label={skin.xp.label} value={wallet.xp} />}
      {skin.showStreak && (
        <Chip icon={"🔥"} label="day streak" value={wallet.streakDays} />
      )}
    </div>
  );
}
...
export function SectionTitle({ children }: { children: ReactNode }): ReactNode {
  const t = useUiTokens();
  return (
    <h2 style={{ margin: "20px 0 10px", fontSize: `calc(1.05rem * ${t.fontScale})`, color: "var(--ink)" }}>
      {children}
    </h2>
  );
}
...
export function StateBlock({
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: 0, fontSize: `calc(1rem * ${t.fontScale})` }}>{message}</p>
      {onRetry && kind === "error" ? (
        <ActionButton variant="ghost" onClick={onRetry} ariaLabel="Try again">
          Try again
        </ActionButton>
      ) : null}
    </div>
...
export function useRewardFormat() {
  const skin = useShellState().economySkin;
  return (reward: { coins?: number | null; diamonds?: number | null; xp?: number | null }): string => {
    const parts: string[] = [];
    if (typeof reward.coins === "number" && reward.coins > 0) parts.push(`${skin.soft.icon} ${reward.coins}`);
    if (typeof reward.diamonds === "number" && reward.diamonds > 0)
      parts.push(`${skin.hard.icon} ${reward.diamonds}`);
    if (typeof reward.xp === "number" && reward.xp > 0) parts.push(`${skin.xp.icon} ${reward.xp}`);
    return parts.join("  ");
  };
}
