 ...
 */
function applyDelta(wallet: Wallet, delta: Partial<Wallet>, sign: 1 | -1): void {
  const keys: (keyof Wallet)[] = ["coins", "diamonds", "xp", "streakDays"];
...
export function createMockAuthPort(): AuthPort {
  ...
  function makeProfile(kind: string, displayName: string, premium: boolean): PlayerProfile {
    counter += 1;
    return {
 ...
 */
export function createMockSavePort(): SavePort {
  let state: unknown = null;
  let checkpoint: { worldId: string; levelId: string } | null = null;
  ...
  return {
    ...
    async save(next) {
      ...
      if (
        ...
        "worldId" in next &&
        "levelId" in next &&
        typeof (next as { worldId: unknown }).worldId === "string" &&
        typeof (next as { levelId: unknown }).levelId === "string"
      ) {
        const c = next as { worldId: string; levelId: string };
 ...
 */
export function createMockEconomyPort(initial?: Partial<Wallet>): EconomyPort {
  const wallet: Wallet = { ...emptyWallet(), ...initial };
 ...
 */
export function createMockRewardsPort(): RewardsPort {
 ...
 */
export function createMockLeaderboardPort(): LeaderboardPort {
  function row(rank: number): { rank: number; playerId: string; displayName: string; score: number } {
 ...
 */
export function createMockCommercePort(): CommercePort {
  ...
  return {
    ...
    async purchase(productId) {
      ...
      if (productId === "mock.premium") entitlements.add("premium");
 ...
 */
export function createMockNotificationsPort(): NotificationsPort {
  const scheduled: { id: string; whenMs: number; body: string }[] = [];
 ...
 */
export function createMockContentPort(): ContentPort {
  return {
    async manifest(gameId) {
      return { gameId, version: 1, packs: [{ id: "core", checksum: "mock-core" }] };
    },
    async pack(packId) {
      return { id: packId, checksum: `mock-${packId}`, items: [] as unknown[] };
    },
  };
}
 ...
 */
export function createMockPorts(overrides?: Partial<ShellPorts>): ShellPorts {
  return {
