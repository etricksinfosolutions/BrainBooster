...
export function LaunchStage({ children, ariaLabel, onActivate, reducedMotion, }) {
    ...
    return (_jsxs("div", { className: "shell-launch-stage", role: onActivate ? "button" : "group", "aria-label": ariaLabel, tabIndex: onActivate ? 0 : undefined, onClick: onActivate, onKeyDown: onActivate
            ? (e) => {
            ...
            : undefined, style: style, children: [_jsx(LaunchStyles, {}), children] }));
}
/**
 ...
 */
export function BrandArt({ art, fallbackLabel, size, reducedMotion, bob, }) {
    const uri = assetUri(art);
    const anim = !reducedMotion && bob ? "shell-launch-bob 3.4s ease-in-out infinite" : undefined;
    if (uri) {
        return (_jsx("img", { src: uri, alt: art?.alt ?? fallbackLabel, width: size, height: size, draggable: false, style: {
                width: size,
                height: "auto",
                maxWidth: "70vw",
                objectFit: "contain",
    ...
    return (_jsx("div", { role: "img", "aria-label": fallbackLabel, style: {
            width: size,
            height: size,
            maxWidth: "70vw",
            display: "flex",
...
export function MascotBadge({ mascot, size, reducedMotion, }) {
    if (!mascot)
        return null;
    const uri = assetUri(mascot.art);
    const anim = reducedMotion ? undefined : "shell-launch-bob 3s ease-in-out infinite";
    if (uri) {
        return (_jsx("img", { src: uri, alt: mascot.name, width: size, height: size, draggable: false, style: {
                width: size,
                height: size,
                objectFit: "contain",
                borderRadius: "50%",
    ...
    return (_jsx("div", { role: "img", "aria-label": mascot.name, style: {
            width: size,
            height: size,
            display: "flex",
            alignItems: "center",
 ...
 */
export function launchButtonStyle(tone, a11y, opts) {
