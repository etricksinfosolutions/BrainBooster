...
function difficultyGuidance(spec) {
    if (!spec.difficultyMix)
        return "Mix easy, medium, and hard questions.";
    const parts = Object.entries(spec.difficultyMix)
        .filter(([, w]) => w > 0)
        .map(([d, w]) => `${d} (~${w})`);
    return `Aim for this rough difficulty balance: ${parts.join(", ")}.`;
}
export function buildUserPrompt(spec, batchSize, avoidPrompts) {
    const lines = [
        `Write ${batchSize} new ${spec.topic} questions.`,
        difficultyGuidance(spec),
    ];
    if (avoidPrompts.length > 0) {
        lines.push(`Do NOT repeat or paraphrase any of these already-used questions:`, ...avoidPrompts.map((p) => `- ${p}`));
    }
    return lines.join("\n");
}
export function buildRequest(spec, batchSize, avoidPrompts) {
    return {
