...
export function defineComposition(composition: unknown): GameComposition {
...
export function referencedPackIds(composition: GameComposition): string[] {
  const ids = new Set<string>();
  for (const world of composition.worlds)
    for (const level of world.levels) for (const ref of level.activities) ids.add(ref.packId);
  return [...ids];
}

/** Total level count across all worlds — the game's progression length. */
export function levelCount(composition: GameComposition): number {
